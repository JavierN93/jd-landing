...
export function AlertDialog({ title, message, type, okCaption, onClose, closeDialog, actions }: Props) {
  ...
  return (<div className="max-w-full sm:w-500 p-20">
    ...
    <div className="flex justify-center py-10">
      {type === DialogType.Alert &&
        <button className="btn btn-primary btn-sm-block" onClick={() => closeWithResult(DialogResult.Ok)}>{okCaption}</button>
      }
      {type === DialogType.Confirmation &&(<>
        <button className="btn btn-danger mr-30" onClick={() => closeWithResult(DialogResult.Yes)}>Yes</button>
        <button className="btn btn-info " onClick={() => closeWithResult(DialogResult.No)}>No</button>
      </>)}
