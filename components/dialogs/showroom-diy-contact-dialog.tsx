...
export function ShowroomDIYContactDialog({ onClose, closeDialog }: Props) {
  ...
  const form = useFormik({
    ...
    onSubmit: async (values: {
      ...
      try {
        setIsLoading(true);
        gaService.event('Request Submitted', 'Showroom DIY Contact Form Submitted');
        await leadApiService.showRoomDIYContact(values);
  ...
  return (
    <div className="w-full sm:w-505 max-h-100vh">
      ...
      <h5 className="text-primary text-center text-24 font-medium mt-0 mb-20 mx-30 sm:mx-60">Contact Us</h5>
      <form className="w-350 mx-auto mb-40 px-20 pb-30 md:pb-0" onSubmit={form.handleSubmit}>
        ...
        <PhoneInput name="phone" label="Phone Number" placeholder="(300) 448 5543" value={form.values.phone}
                    onChange={form.handleChange}/>
        <TextArea name="comment" label="Tell us what you would like a quote on?" placeholder="" rows={4}
...
ShowroomDIYContactDialog.defaultProps = {
  onClose: () => {
  },
  closeDialog: () => {
  },
